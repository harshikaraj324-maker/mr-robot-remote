...
type Env = {
  ...
  API_SECRET?: string;           // x-api-key guard for GET + DELETE + PATCH
...
// Reuse the same neon HTTP client per isolate — avoids new HTTPS connection each request
const _neonClients: Record<string, ReturnType<typeof neon>> = {};
function getNeon(url: string) {
  return _neonClients[url] || (_neonClients[url] = neon(url));
}
function getDb(env: Env) {
  return drizzle(getNeon(env.NEON_DATABASE_URL), { schema: { apps, devices, messages, formData } });
}

// =================== SCHEMA INIT (lazy, once-per-worker) ===================
let schemaInitPromise: Promise<void> | null = null;

// ── In-memory caches (per worker isolate; resets on cold start) ──────────
// Master PIN: fetched from DB once per isolate, cached 5 min
let _pinCache: { v: string; exp: number } | null = null;
function pinCacheGet(): string | null {
  if (_pinCache && Date.now() < _pinCache.exp) return _pinCache.v;
  return null;
}
function pinCacheSet(v: string) { _pinCache = { v, exp: Date.now() + 5 * 60_000 }; }
function pinCacheInvalidate() { _pinCache = null; }
// App rows: cached 2 min per appId
const _appCache: Record<string, { row: Record<string, unknown>; exp: number }> = {};
function appCacheGet(id: string) {
  const e = _appCache[id];
  return (e && Date.now() < e.exp) ? e.row : null;
}
function appCacheSet(id: string, row: Record<string, unknown>) {
  _appCache[id] = { row, exp: Date.now() + 2 * 60_000 };
}
function appCacheInvalidate(id?: string) {
  if (id) delete _appCache[id]; else Object.keys(_appCache).forEach(k => delete _appCache[k]);
}

// Simple in-memory rate limiter for master PIN brute-force protection
const _pinAttempts = new Map<string, { count: number; resetAt: number }>();
function checkRateLimit(ip: string): boolean {
  const now = Date.now();
  const entry = _pinAttempts.get(ip);
  if (!entry || now > entry.resetAt) {
    _pinAttempts.set(ip, { count: 1, resetAt: now + 15 * 60 * 1000 });
    return true;
  }
...
function clearRateLimit(ip: string): void { _pinAttempts.delete(ip); }
async function ensureSchema(env: Env): Promise<void> {
  if (schemaInitPromise) return schemaInitPromise;
  schemaInitPromise = (async () => {
    const sqlClient = getNeon(env.NEON_DATABASE_URL);
...
// ── API Key middleware ──────────────────────────────────────────────────────
// POST is free (Android app writes data); GET + DELETE + PATCH require x-api-key.
// Exempt: healthz, events (WS), admin/sessions (has own session auth), apps/:id (pre-login check).
app.use("*", async (c, next) => {
  const method = c.req.method;
  if (method === "POST" || method === "OPTIONS") return next(); // Android app POST free; OPTIONS = CORS preflight
  const path = new URL(c.req.url).pathname;
  // Always open — healthz, WebSocket, session login/logout, app-status check before login
  if (
    path === "/api/healthz" ||
    path === "/api/events" ||
    path.startsWith("/api/admin/sessions") ||          // session list + logout use session-id auth
    (path.startsWith("/api/apps/") && method === "GET") // only GET is pre-login app-status check
  ) return next();
  const secret = c.env.API_SECRET;
  if (!secret) return next();                          // not configured — open
  const provided = c.req.header("x-api-key") ?? c.req.query("key") ?? "";
  if (provided !== secret) return c.json({ error: "Invalid API key" }, 401);
  return next();
});
// ────────────────────────────────────────────────────────────────────────────
...
app.patch("/api/apps/:appId", async (c) => {
  ...
  if (body.name !== undefined || body.pin !== undefined || body.status !== undefined) {
    const guard = await checkMasterPin(c as never);
    if (guard) return guard;
  }
...
app.post("/api/apps/:appId/verify-pin", async (c) => {
  const db = getDb(c.env);
  const sqlClient = getNeon(c.env.NEON_DATABASE_URL);
  const body = await c.req.json() as { pin?: string };
  if (!body.pin) return c.json({ error: "PIN required" }, 400);
  const appId = c.req.param("appId");
  // Try in-memory cache first — avoids a DB round-trip on warm isolates
  let cached = appCacheGet(appId);
  if (!cached) {
    const [fetched] = await db.select().from(apps).where(eq(apps.appId, appId)).limit(1);
    if (fetched) { appCacheSet(appId, fetched as Record<string, unknown>); cached = fetched as Record<string, unknown>; }
  }
...
app.get("/api/devices", async (c) => {
  ...
  // appId/userId are optional filters — API key middleware handles auth
  const limitParam = c.req.query("limit");
  const devLimit = limitParam == null ? 500 : Math.max(0, Math.min(2000, parseInt(limitParam, 10) || 0));
  const where = appId ? eq(devices.appId, appId) : userId ? eq(devices.userId, userId) : undefined;
  let q = where
    ? db.select().from(devices).where(where).orderBy(desc(devices.updatedAt))
    : db.select().from(devices).orderBy(desc(devices.updatedAt));
  if (devLimit > 0) q = q.limit(devLimit) as typeof q;
  const rows = await q;
...
app.get("/api/messages", async (c) => {
  ...
  // Filters are optional — API key middleware handles auth; no filter = return all (capped)
...
app.get("/api/data", async (c) => {
  ...
  const appId = c.req.query("appId");
  const deviceId = c.req.query("deviceId");
  // appId is optional filter — API key middleware handles auth
  const limitParam = c.req.query("limit");
  const offsetParam = c.req.query("offset");
  const rowLimit = limitParam == null ? 200 : Math.max(0, Math.min(2000, parseInt(limitParam, 10) || 0));
  const rowOffset = Math.max(0, parseInt(offsetParam ?? "0", 10) || 0);
  const where = appId && deviceId ? and(eq(formData.appId, appId), eq(formData.deviceId, deviceId))
    : appId ? eq(formData.appId, appId)
    : deviceId ? eq(formData.deviceId, deviceId)
    : undefined;
  let q = where
    ? db.select().from(formData).where(where).orderBy(desc(formData.submittedAt))
    : db.select().from(formData).orderBy(desc(formData.submittedAt));
  if (rowLimit > 0) q = q.limit(rowLimit).offset(rowOffset) as typeof q;
  const rows = await q;
...
app.delete("/api/data/:id", async (c) => {
  ...
  // peek appId first, then auth
  const [peek] = await db.select({ appId: formData.appId }).from(formData).where(eq(formData.id, id)).limit(1);
  if (!peek) return c.json({ error: "Not found" }, 404);
...
app.delete("/api/data", async (c) => {
  ...
  if (c.req.query("confirm") !== "yes") return c.json({ error: "Add ?confirm=yes to confirm bulk delete" }, 400);
...
app.delete("/api/messages/:id", async (c) => {
  ...
  const id = Number(c.req.param("id"));
  if (Number.isNaN(id)) return c.json({ error: "Invalid id" }, 400);
  const [peek] = await db.select({ appId: messages.appId }).from(messages).where(eq(messages.id, id)).limit(1);
  if (!peek) return c.json({ error: "Not found" }, 404);
...
// Bulk delete ALL messages for an appId (Settings → Delete All SMS)
app.delete("/api/messages", async (c) => {
  const db = getDb(c.env);
  const appId = c.req.query("appId");
  if (!appId) return c.json({ error: "appId required" }, 400);
  const rows = await db.delete(messages).where(eq(messages.appId, appId)).returning({ id: messages.id });
  await broadcast(c.env, "messages_bulk_deleted", { appId, count: rows.length });
...
app.delete("/api/devices/:deviceId", async (c) => {
  ...
  const [peek] = await db.select({ appId: devices.appId }).from(devices).where(eq(devices.deviceId, deviceId)).limit(1);
  if (!peek) return c.json({ error: "Device not found" }, 404);
  // Auth handled by API key middleware — x-api-key required for all DELETE requests
...
// ------- DEVICE-SCOPED DATA ALIAS (Android app posts here) -------
// Android app: POST {BACKEND_ROOT}/api/device/{appId}/data
app.post('/api/device/:appId/data', async (c) => {
  const db = getDb(c.env);
  const appId = c.req.param('appId');
  const body = await c.req.json() as { deviceId?: string; data?: Record<string, unknown> };
  const deviceId = body.deviceId ?? '';
  if (!deviceId) return c.json({ error: 'deviceId is required' }, 400);
  if (!body.data || typeof body.data !== 'object' || Array.isArray(body.data)) {
    return c.json({ error: 'data must be a JSON object' }, 400);
  }
  const [row] = await db.insert(formData).values({ appId, deviceId, data: body.data }).returning();
  const mapped = mapFormData(row);
  await broadcast(c.env, 'form_data_added', { appId, formData: mapped });
  return c.json(mapped, 201);
});
...
async function getMasterPin(env: Env): Promise<string> {
  const cached = pinCacheGet();
  if (cached !== null) return cached;
  const rows = await getNeon(env.NEON_DATABASE_URL)(`SELECT value FROM settings WHERE key = 'master_pin'`) as Array<{ value: string }>;
  const v = rows[0]?.value ?? "master1234";
  pinCacheSet(v);
  return v;
}
async function checkMasterPin(c: Parameters<typeof app.use>[1] extends (c: infer C, n: () => Promise<void>) => unknown ? C : never): Promise<Response | null> {
  ...
  const stored = await getMasterPin(c.env);
  if (pin !== stored) return c.json({ error: "Wrong Master PIN" }, 401);
  return null;
}

// Validates x-session-id header — must be an active session for the given appId
async function checkSessionAuth(c: Parameters<typeof app.use>[1] extends (c: infer C, n: () => Promise<void>) => unknown ? C : never, _appId: string): Promise<Response | null> {
  const sessionId = c.req.header("x-session-id") ?? "";
  if (!sessionId) return c.json({ error: "x-session-id header required" }, 401);
  const sqlClient = getNeon(c.env.NEON_DATABASE_URL);
  const rows = await sqlClient(
    `SELECT id FROM admin_sessions WHERE id = $1 AND last_active > NOW() - INTERVAL '4 hours'`,
    [sessionId],
  ) as Array<{ id: string }>;
  if (rows.length === 0) return c.json({ error: "Session expired. Please logout and login again." }, 401);
  return null;
}
...
app.post("/api/admin/verify-master-pin", async (c) => {
  ...
  const body = await c.req.json() as { pin?: string };
  if (!body.pin) return c.json({ error: "PIN required" }, 400);
  const stored = await getMasterPin(c.env); // cached — no DB hit on warm isolate
...
app.patch("/api/admin/master-pin", async (c) => {
  ...
  pinCacheInvalidate(); // force fresh DB read next login
  return c.json({ ok: true });
});
...
app.post("/api/admin/sessions", async (c) => {
  ...
  // Single CTE: find existing session OR create new — 2 queries → 1 round-trip
  const newId = crypto.randomUUID();
  const rows = await sqlClient(
    `WITH found AS (
       SELECT id FROM admin_sessions
       WHERE user_agent = $1 AND ip = $2 AND app_id = $3
       ORDER BY last_active DESC LIMIT 1
     ), upd AS (
       UPDATE admin_sessions SET last_active = NOW()
       WHERE id = (SELECT id FROM found)
       RETURNING id
     ), ins AS (
       INSERT INTO admin_sessions (id, user_agent, ip, device, app_id)
       SELECT $4, $1, $2, $5, $3
       WHERE NOT EXISTS (SELECT 1 FROM found)
       RETURNING id
     )
     SELECT id FROM upd UNION ALL SELECT id FROM ins`,
    [ua, ip, appId, newId, parseDevice(ua)],
  ) as Array<{ id: string }>;
  return c.json({ sessionId: rows[0]?.id ?? newId });
});
