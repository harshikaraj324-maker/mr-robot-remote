        ...
        <div style={{ display: "flex", gap: 12, fontSize: 11, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <DeleteIconButton
            ...
            onConfirm={async () => {
              ...
              if (!r.ok) throw new Error(`Delete failed (${r.status}). Please try again.`);
            }}
          />
        </div>
      </div>
...
function HomePage({
  ...
  // Sort once when messages array changes — avoids re-sorting crores on every keypress
  const sortedMsgs = useMemo(() =>
    [...messages].sort((a, b) => new Date(b.receivedAt).getTime() - new Date(a.receivedAt).getTime()),
  [messages]);

  // Filter separately — only reruns on search/deviceMap change, not on every new message
  const allMsgs = useMemo(() => {
    const q = search.toLowerCase().trim();
    if (!q) return sortedMsgs;
    return sortedMsgs.filter(m => {
                  ...
                  {devForm.length > 0 && (
                    <div style={{ display: "flex", justifyContent: "flex-end", padding: "6px 10px", background: H, borderTop: `1px solid ${H}` }}>
                      <DeleteIconButton
                        ...
                        onConfirm={async () => {
                          const r = await fetch(`/api/data?appId=${encodeURIComponent(device.appId)}&deviceId=${encodeURIComponent(device.deviceId)}&confirm=yes`, { method: "DELETE" });
                          if (!r.ok) throw new Error(`Delete failed (${r.status}). Please try again.`);
                        }}
                      />
                    </div>
  ...
  async function handleDeleteDevice(deviceId: string) {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        alert(err.error ?? `Delete failed (${r.status}). Please try again.`);
        return;
      }
      setDeletedIds(prev => new Set([...prev, deviceId]));
    } catch {
      alert("Network error. Device delete failed. Please try again.");
    } finally {
      setDeleting(false);
...
function SettingsPage({ appId, isDark, onToggleDark, devices, onLogout }: {
  ...
  // Delete All SMS state
  const [delAllSmsStep, setDelAllSmsStep] = useState<"idle"|"confirm"|"pin"|"deleting"|"done">("idle");
  const [delAllSmsPin, setDelAllSmsPin] = useState("");
  const [delAllSmsPinErr, setDelAllSmsPinErr] = useState("");
  const [delAllSmsCount, setDelAllSmsCount] = useState(0);
  ...
  return (
    <div style={{ padding: 10, display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {/* ── Delete All SMS ── */}
      <div style={{ background: t.card, borderRadius: 10, border: `1px solid ${t.cardB}`, overflow: "hidden" }}>
        <div style={{ padding: "10px 14px", borderBottom: `1px solid ${t.hdrB}`, fontSize: 12, fontWeight: 700, color: "#ef4444" }}>🗑 Danger Zone</div>
        <div style={{ padding: "14px 16px" }}>
          {delAllSmsStep === "idle" && (
            <button
              onClick={() => setDelAllSmsStep("confirm")}
              style={{ width: "100%", padding: "11px 0", borderRadius: 9, border: "1.5px solid #ef4444", background: "transparent", color: "#ef4444", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              🗑 Delete All SMS of this App
            </button>
          )}
          {delAllSmsStep === "confirm" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ fontSize: 13, color: t.txt, fontWeight: 600, textAlign: "center" }}>
                Are you sure? This will permanently delete <b>ALL SMS messages</b> for <code style={{ background: t.hdrB, padding: "1px 5px", borderRadius: 4 }}>{appId}</code>.<br/>
                <span style={{ color: "#ef4444", fontSize: 11 }}>This action cannot be undone.</span>
              </div>
              <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                <button
                  onClick={() => setDelAllSmsStep("pin")}
                  style={{ flex: 1, padding: "10px 0", borderRadius: 9, background: "#ef4444", border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                  Yes, Continue
                </button>
                <button
                  onClick={() => setDelAllSmsStep("idle")}
                  style={{ flex: 1, padding: "10px 0", borderRadius: 9, background: "transparent", border: `1.5px solid ${t.cardB}`, color: t.txt2, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                  Cancel
                </button>
              </div>
          ...
          {delAllSmsStep === "pin" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ fontSize: 12, color: t.txt2, fontWeight: 600 }}>Enter your PIN to confirm deletion:</div>
              <input
                type="password"
                value={delAllSmsPin}
                onChange={e => { setDelAllSmsPin(e.target.value); setDelAllSmsPinErr(""); }}
                placeholder="Enter PIN"
                autoFocus
                onKeyDown={e => { if (e.key === "Enter" && delAllSmsPin) void (async () => {
                  setDelAllSmsPinErr(""); setDelAllSmsStep("deleting");
                  try {
                    const vr = await fetch(`/api/apps/${appId}/verify-pin`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ pin: delAllSmsPin }) });
                    if (!vr.ok) { setDelAllSmsPinErr("Wrong PIN. Try again."); setDelAllSmsPin(""); setDelAllSmsStep("pin"); return; }
                    const dr = await fetch(`/api/messages?appId=${encodeURIComponent(appId)}`, { method: "DELETE" });
                    if (!dr.ok) throw new Error("Delete failed");
                    const { deleted } = await dr.json() as { deleted: number };
                    setDelAllSmsCount(deleted); setDelAllSmsStep("done");
                    setTimeout(() => setDelAllSmsStep("idle"), 4000);
                  } catch { setDelAllSmsPinErr("Error. Please try again."); setDelAllSmsStep("pin"); }
                })(); }}
                style={{ padding: "10px 12px", borderRadius: 9, border: `1.5px solid ${delAllSmsPinErr ? "#ef4444" : t.cardB}`, background: t.bg, color: t.txt, fontSize: 13, outline: "none" }}
              />
              {delAllSmsPinErr && <div style={{ color: "#ef4444", fontSize: 11, fontWeight: 600 }}>{delAllSmsPinErr}</div>}
              <div style={{ display: "flex", gap: 8 }}>
                <button
                  disabled={!delAllSmsPin}
                  onClick={async () => {
                    setDelAllSmsPinErr(""); setDelAllSmsStep("deleting");
                    try {
                      const vr = await fetch(`/api/apps/${appId}/verify-pin`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ pin: delAllSmsPin }) });
                  ...
                  style={{ flex: 1, padding: "10px 0", borderRadius: 9, background: !delAllSmsPin ? "#334155" : "#ef4444", border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: !delAllSmsPin ? "default" : "pointer" }}>
                  Delete All SMS
                </button>
                <button
                  onClick={() => { setDelAllSmsStep("idle"); setDelAllSmsPin(""); setDelAllSmsPinErr(""); }}
                  style={{ flex: 1, padding: "10px 0", borderRadius: 9, background: "transparent", border: `1.5px solid ${t.cardB}`, color: t.txt2, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                  Cancel
                </button>
              </div>
          ...
          {delAllSmsStep === "deleting" && (
            <div style={{ textAlign: "center", padding: "12px 0", color: "#ef4444", fontWeight: 700, fontSize: 13 }}>
              🗑 Deleting all SMS…
            </div>
          )}
          {delAllSmsStep === "done" && (
            <div style={{ textAlign: "center", padding: "12px 0", color: "#22c55e", fontWeight: 700, fontSize: 13 }}>
              ✓ Deleted {delAllSmsCount} messages successfully!
            </div>
          )}
        </div>
      </div>
...
export default function WebDashboard() {
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    const FIRST_PAGE = 200;   // Fast first paint — dashboard renders immediately
    const PAGE_SIZE = 500;    // Background chunks for older messages
    const MAX_MESSAGES = 5000; // Safety cap — browser can't hold crores in RAM
    try {
      ...
      const [dRes, mRes, fRes] = await Promise.all([
        fetch(`/api/devices?appId=${appId}&limit=300`, { headers: h, signal: controller.signal }),
        fetch(`/api/messages?appId=${appId}&limit=${FIRST_PAGE}&offset=0`, { headers: h, signal: controller.signal }),
        fetch(`/api/data?appId=${appId}&limit=200`, { headers: h, signal: controller.signal }),
      ]);
      ...
      // Append page-by-page; dedupe by id (O(1) Map lookup — safe for large datasets).
      // Stops at MAX_MESSAGES so the browser never holds crores of rows in RAM.
      (async () => {
        let offset = (firstM as DbMessage[]).length;
        if (offset < FIRST_PAGE) return; // first page was the last page
        for (;;) {
          try {
            if (offset >= MAX_MESSAGES) break; // safety cap
            ...
            setMessages(prev => {
              if (prev.length >= MAX_MESSAGES) return prev; // already at cap
              const seen = new Map(prev.map(m => [m.id, true])); // O(1) lookup
  ...
  // WebSocket — robust auto-reconnect with visibilitychange + online events
  useEffect(() => {
    if (!authed) return;
    ...
    function connect() {
      ...
      const wsKey = encodeURIComponent((import.meta as Record<string,any>).env?.VITE_API_SECRET ?? "");
      const url = `${proto}//${window.location.host}/api/events${wsKey ? "?key=" + wsKey : ""}`;
      ...
      ws.onmessage = (e) => {
        ...
        } else if (event === "messages_bulk_deleted") {
          const payload = data as { appId: string; count: number };
          if (payload.appId !== appId) return;
          setMessages([]);  // all messages of this appId deleted
      ...
      ws.onclose = () => {
        ...
        if (closed) return;
        // backoff: 1.5s → 3s → 6s → cap 8s (fast enough to feel instant)
    ...
    // Reconnect immediately when tab becomes visible or internet comes back
    function onVisible() { if (document.visibilityState === "visible") { retryDelay = 1500; if (retryTimer) { clearTimeout(retryTimer); retryTimer = null; } connect(); } }
    function onOnline()  { retryDelay = 1500; if (retryTimer) { clearTimeout(retryTimer); retryTimer = null; } connect(); }
