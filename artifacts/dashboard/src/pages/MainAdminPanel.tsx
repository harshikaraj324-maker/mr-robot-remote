...
function Dashboard({ masterPin, onLogout, onPinChanged }: { masterPin: string; onLogout: () => void; onPinChanged: (p: string) => void }) {
  ...
  const fetchApps = useCallback(async () => {
    try {
      ...
      if (!r.ok) return;
      const apps = await r.json() as App[];
  ...
  function copyUrl(app: App) {
    const url = `https://mr-robot.s39452363.workers.dev/d/${app.appId}`;
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {filteredApps.map((app, idx) => {
              ...
              const sessionsUsed = app.activeSessions ?? 0;
              const sessionFull = sessionsUsed >= app.loginLimit;
              const accentLeft = isActive ? T.accent : T.border;
              return (
                <div key={app.appId} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 220 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 15, fontWeight: 800, color: T.text }}>{app.name}</span>
                      <span style={{
                        fontSize: 10, fontWeight: 700, letterSpacing: 0.5,
                        color: isActive ? T.green : T.red,
                        background: isActive ? T.green + "18" : T.red + "18",
                        padding: "2px 8px", borderRadius: 99, textTransform: "uppercase",
                      }}>
                        {isActive ? "● Active" : "● Disabled"}
                      </span>
                      {idx === 0 && <span style={{ fontSize: 10, fontWeight: 700, color: T.accent, background: T.accent + "18", padding: "2px 8px", borderRadius: 99 }}>Latest</span>}
                    </div>

                    {/* Row 2: ID + PIN inline */}
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 14, marginBottom: 5 }}>
                      <span style={{ fontSize: 11, color: T.muted, fontFamily: "monospace" }}>
                        ID: <span style={{ color: "#a5b4fc", fontWeight: 600 }}>{app.appId}</span>
                      </span>
                      <span style={{ fontSize: 11, color: T.muted, fontFamily: "monospace" }}>
                        PIN: <span style={{ color: T.text, fontWeight: 700, letterSpacing: 3 }}>{app.pin}</span>
                      </span>
                    </div>

                    {/* Row 3: sessions pill + created */}
                    <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                      <div style={{
                        display: "flex", alignItems: "center", gap: 5,
                        background: sessionFull ? T.red + "18" : "#0f172a",
                        border: `1px solid ${sessionFull ? T.red + "44" : T.border}`,
                        borderRadius: 8, padding: "3px 10px",
                      }}>
                        <span style={{ fontSize: 10, color: T.muted }}>Sessions</span>
                        <span style={{ fontSize: 12, fontWeight: 800, color: sessionFull ? T.red : T.green }}>
                          {sessionsUsed}/{app.loginLimit}
                        </span>
                      </div>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                    <button onClick={() => copyUrl(app)} style={{
                      padding: "7px 12px", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap", border: "none",
                      background: copyMsg[app.appId] ? T.green + "22" : "#334155",
                      color: copyMsg[app.appId] ? T.green : T.text,
                    }}>
                      {copyMsg[app.appId] || "📋 URL"}
                    </button>
                    <button onClick={() => setEditApp(app)} style={{
                      padding: "7px 12px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", border: "none",
                      background: T.accent + "22", color: T.accent,
                    }}>✏️ Edit</button>
                    <button onClick={() => void logoutAll(app)} disabled={logoutAllId === app.appId} style={{
                      padding: "7px 12px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: logoutAllId === app.appId ? "wait" : "pointer", whiteSpace: "nowrap", border: "none",
                      background: "#f97316" + "22", color: "#f97316", opacity: logoutAllId === app.appId ? 0.6 : 1,
                    }}>
                      {logoutAllId === app.appId ? "…" : "🔓 Logout All"}
                    </button>
                    <button onClick={() => toggleStatus(app)} disabled={togglingId === app.appId} style={{
                      padding: "7px 12px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", border: "none",
                      background: isActive ? T.yellow + "22" : T.green + "22",
                      color: isActive ? T.yellow : T.green,
                    }}>
                      {togglingId === app.appId ? "…" : isActive ? "Disable" : "Enable"}
                    </button>
                    <button onClick={() => deleteApp(app)} disabled={deletingId === app.appId} style={{
                      padding: "7px 10px", borderRadius: 8, fontSize: 13, cursor: "pointer", border: "none",
